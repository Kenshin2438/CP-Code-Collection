...
void solve() {
  ...
  vector<vector<mint>> ans0(n, vector<mint>(n));
  vector<vector<mint>> ans1(n, vector<mint>(n));
  vector<vector<mint>> way0(n, vector<mint>(n));
  vector<vector<mint>> way1(n, vector<mint>(n));
  mint ans = 0;
  for (int i = 0; i < n; i++) {
    for (int k = 0, j = 0; j < i; j++) {
      ...
      if (j - 1 >= 0) {
        ...
        if (k == 0) {
          ...
          debug(k);
          ans0[i][j] += ans0[j][j - 1] - ans0[j][k - 1];
          ans0[i][j] += ans1[j][j - 1] - ans1[j][k - 1];
          way0[i][j] += way0[j][j - 1] - way0[j][k - 1];
          way0[i][j] += way1[j][j - 1] - way1[j][k - 1];

          way1[i][j] += way0[j][k - 1] + way1[j][k - 1];
          ans1[i][j] += ans0[j][k - 1] + ans1[j][k - 1] + way0[j][k - 1];
        }
      }

      // for (int k = j - 1; k >= 0; k--) {
      //   if (a[i] - a[j] >= a[j] - a[k]) {
      //     way0[i][j] += way0[j][k] + way1[j][k];
      //     ans0[i][j] += ans0[j][k] + ans1[j][k];
      //   } else {
      //     debug(k);
      //     way1[i][j] += way0[j][k] + way1[j][k];
      //     ans1[i][j] += ans0[j][k] + ans1[j][k] + way0[j][k];
      //   }
      // }
      ans += ans0[i][j] + ans1[i][j];
    }
    partial_sum(all(ans0[i]), begin(ans0[i]));
    partial_sum(all(ans1[i]), begin(ans1[i]));
    partial_sum(all(way0[i]), begin(way0[i]));
    partial_sum(all(way1[i]), begin(way1[i]));
  }
  cout << ans << '\n';
}
