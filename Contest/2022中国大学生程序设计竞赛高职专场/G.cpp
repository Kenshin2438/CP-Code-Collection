...
void solve() {
  ...
  mint ans = 0;
  for (int i = 1; i <= 9; i++) if (cnt[i] != 0) {
    vector<vector<mint>> dp(n, vector<mint>(10 * n));
    dp[0][0] = 1;
    for (int j = 1; j <= 9; j++) {
      int m = cnt[j] - (i == j);
      while (m--) {
        for (int k = n - 1; k >= 1; k--) {
          for (int v = 0; v + j < 10 * n; v++) {
            dp[k][v + j] += dp[k - 1][v];
          }
        }
      }
    ...
    for (int k = 0; k <= n - 1; k++) {
      for (int v = 0; v < 10 * n; v++) {
        ans += dp[k][v] * S[i] * mint(10).pow(v) * comb.fac(k) * comb.fac(n - 1 - k);
      }
    }
  }
  cout << ans << '\n';
