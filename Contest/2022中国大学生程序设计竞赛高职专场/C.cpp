...
void solve() {
  ...
  mint ans = 0;
  for (int i = 0; i < (int) AA.size(); i += 2) {
    if (AA[i] != 0 && B[i / 2] != 0) ans += (AA[i] - A[i / 2]) / 2;
  }
  for (int i = 0; i < (int) BB.size(); i += 2) {
    if (BB[i] != 0 && A[i / 2] != 0) ans += (BB[i] - B[i / 2]) / 2;
  }

  int d = y1 - y0;
  int d2 = d * d;
  map<long long, int> sqrt;
  ...
  for (long long i = 0; i <= N; ++i) {
    ...
    if (sqrt.count(k)) {
      int t = sqrt[k];
      match[t] = i, poss.emplace_back(t, i);
    }
  }
  for (int p = 0; p < 2; ++p) {
    for (int i = 0; i <= N; ++i) {
