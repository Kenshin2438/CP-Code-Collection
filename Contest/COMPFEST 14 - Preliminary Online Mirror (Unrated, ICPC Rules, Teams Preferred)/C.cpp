...
void solve() {
  ...
  if (SUM & 1LL) return cout << mint(m).pow(n) << '\n', void();
  SUM >>= 1;

  int N0 = 0;
  for (int i = 0; i < n; i++) {
    N0 += binary_search(all(D), D[i] - SUM);
  }

  debug(N0);

  if (N0 > m) return cout << 0 << '\n', void();

  Combi<mint> comb;
  mint ans = 0;
  for (int t = 0; t <= N0; t++) {
    ans += comb.C(N0, t) * comb.P(m, t) * mint(m - t).pow(n - 2 * N0) * comb.P(m - t, 2).pow(N0 - t);
  }
  cout << ans << '\n';
}
