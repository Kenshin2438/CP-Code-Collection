...
void solve() {
  ...
  dp[0][0] = dp[0][1] = 1;
  for (int i = 1; i < n; i++) {
    if (cards[i].first != cards[i - 1].first) dp[i][0] += dp[i - 1][0]; 
    if (cards[i].first != cards[i - 1].second) dp[i][0] += dp[i - 1][1]; 

    if (cards[i].second != cards[i - 1].first) dp[i][1] += dp[i - 1][0]; 
    if (cards[i].second != cards[i - 1].second) dp[i][1] += dp[i - 1][1]; 
  }
