...
struct Poly : public vector<Mint> {
  ...
  Poly inv(int n) const {
    ...
    for (int h = 2, m = 1; m < n; ++h, m <<= 1) {
      ...
      memcpy(polyWork0, data(), min(m << 1, size()) * sizeof(Mint));
      memset(polyWork0 + min(m << 1, size()), 0, ((m << 1) - min(m << 1,
  size())) * sizeof(Mint));
      {
        Mint aa = 1;
        for (int i = 0; i < m; ++i) { polyWork0[(m << 1) + i] = aa *
  polyWork0[i]; aa *= a; } for (int i = 0; i < m; ++i) { polyWork0[(m << 1) + i]
  += aa * polyWork0[m + i]; aa *= a; }
      }
      fft(polyWork0, m << 1);  // 2 E(n)
      fft(polyWork0 + (m << 1), m);  // 1 E(n)
      memcpy(polyWork1, fs.data(), min(m << 1, n) * sizeof(Mint));
      memset(polyWork1 + min(m << 1, n), 0, ((m << 1) - min(m << 1, n)) *
  sizeof(Mint));
      {
        Mint aa = 1;
        for (int i = 0; i < m; ++i) { polyWork1[(m << 1) + i] = aa *
  polyWork1[i]; aa *= a; } for (int i = 0; i < m; ++i) { polyWork1[(m << 1) + i]
  += aa * polyWork1[m + i]; aa *= a; }
      }
      fft(polyWork1, m << 1);  // 2 E(n)
      fft(polyWork1 + (m << 1), m);  // 1 E(n)
      for (int i = 0; i < (m << 1) + m; ++i) polyWork0[i] *= polyWork1[i] *
  polyWork1[i]; invFft(polyWork0, m << 1);  // 2 E(n) invFft(polyWork0 + (m <<
  1), m);  // 1 E(n)
      // 2 f0 + (-f2), (-f1) + (-f3), 1^(1/4) (-f1) - (-f2) - 1^(1/4) (-f3)
      {
        Mint bb = 1;
        for (int i = 0, i0 = min(m, n - m); i < i0; ++i) {
          unsigned x = polyWork0[i].x + (bb * polyWork0[(m << 1) + i]).x + MO2 -
  (fs[i].x << 1);  // < 4 MO fs[m + i] = Mint(static_cast<unsigned long
  long>(FFT_ROOTS[2].x) * x) - polyWork0[m + i]; fs[m + i].x = ((fs[m + i].x &
  1) ? (fs[m + i].x + MO) : fs[m + i].x) >> 1; bb *= b;
        }
      }
    }
    return fs;
  ...
  */
  ...
  // f <- f - (log f - t) / (1 / f)
  //   =  f - (I ((D f) / f) - t) f
  //   == f - (I ((D f) / f + (f g - 1) ((D f) / f - D (t mod x^m))) - t) f (mod
  //   x^(2m)) =  f - (I (g (D f - f D (t mod x^m)) + D (t mod x^m)) - t) f
  // g <- g - (f g - 1) g
...
struct SubproductTree {
  int logN, n, nn;
  vector<Mint> xs;
  // [DFT_4((X-xs[0])(X-xs[1])(X-xs[2])(X-xs[3]))]
  // [(X-xs[0])(X-xs[1])(X-xs[2])(X-xs[3])mod X^4] [ DFT_4((X-xs[0])(X-xs[1]))
  // ] [         DFT_4((X-xs[2])(X-xs[3]))         ] [   DFT_2(X-xs[0])   ] [
  // DFT_2(X-xs[1])   ] [   DFT_2(X-xs[2])   ] [   DFT_2(X-xs[3])   ]
  vector<Mint> buf;
  vector<Mint *> gss;
  ...
  // ((3/2) ceil(log_2 n) + O(1)) E(n) + 10 E(|f|) + 3 E(|f| + 2^(ceil(log_2
  // n)))
  ...
  Poly interpolate(const vector<Mint> &ys) const {
    ...
    const vector<Mint> denoms =
        multiEval(gs);  // ((3/2) ceil(log_2 n) + O(1)) E(n)
...
int main() {
  const int N = readInt();
  const long long K = readLong();

  if (N & 1) {
    writeInt(0), putchar('\n');
  } else {
    vector<Mint> Catalan(N + 1);
    Catalan[0] = 1;
    for (int i = 1; i <= N; i++) {
    ...
    Mint ifac = 1;
    for (int i = 0; i <= N; i++) {
      if (i > 0) ifac *= Mint(i).inv();
      if (~i & 1) A[i] = Catalan[i / 2] * ifac;
    }
    const Mint fac = ifac.inv();

    const Poly B = A.pow(K, N + 1);
    assert(B.size() == N + 1);
    writeInt((B.back() * fac).x), putchar('\n');
  }
  return 0;
}
